...
Oddzielny, samodzielny program do pobierania postow z platformy **X (Twitter)
przez przegladarke** (Playwright + realny DOM x.com) - **bez korzystania z
oficjalnego API**. Domyslnie wyszukuje posty o zadanej firmie/hasle (np.
"Google") powiazane z tematyka finansowa (earnings, akcje, przychody itp.)
opublikowane w ciagu ostatnich **72 godzin**.

## Jak to dziala

1. X wymaga zalogowania, zeby przegladac wyniki wyszukiwania - dlatego program
   sklada sie z dwoch krokow: jednorazowego logowania (`login.js`, przegladarka
   z widocznym oknem) oraz wlasciwego scrapera (`scrape.js`, dziala headless,
...
npm install
npx playwright install chromium   # tylko jesli przegladarka nie jest juz zainstalowana
```
...
Enter. Sesja zostanie zapisana do `auth/storageState.json` i bedzie wielokrotnie
wykorzystywana przez scraper (dopoki nie wygasnie).

> Ten krok wymaga ekranu - uruchamiaj go na swoim komputerze, nie na serwerze
> bez GUI. Plik `auth/storageState.json` mozna potem przeniesc na serwer.
...
node src/scrape.js --query "Tesla" --keywords "earnings,stock,revenue" --lang en
node src/scrape.js --query "Google" --strict          # odrzucaj posty bez slowa finansowego
node src/scrape.js --query "Google" --max-tweets 100 --max-scrolls 40
node src/scrape.js --query "Google" --headful          # pokaz okno przegladarki (wymaga ekranu)
```
...
- To narzedzie odczytuje strukture DOM x.com, ktora **moze sie zmieniac** -
  jesli X zmieni interfejs, selektory w `src/scrape.js` moga wymagac aktualizacji.
